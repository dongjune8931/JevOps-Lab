...
- M4a 첫 통합 테스트, M5 AWS 비교 평가, 선택형 M7 최종 검증은 실행 전에 각각 구성·예상 비용·최대 상한·실행 시간·중단 및 teardown 방법을 승인받는다.
- 크레딧 잔액·만료일·적용 서비스는 아직 확인하지 않았다. 크레딧이 있어도 리소스 생성이나 사용 승인을 받은 것으로 간주하지 않는다.
- 발생한 AWS 사용 비용, 크레딧 차감액, 실제 청구액을 구분하고 Jev API 비용은 별도로 기록한다. 기본 승인 상한은 계속 0원이다.
- 비용 승인이 늦으면 로컬 경로를 유지한다. M3 이후 mock으로 AWS 연결만 먼저 확인하는 경우도 AWS 비용 승인은 필요하다.
- 이번 계획 변경에서는 AWS 계정 접근·리소스 생성·크레딧 사용을 수행하지 않았다.
